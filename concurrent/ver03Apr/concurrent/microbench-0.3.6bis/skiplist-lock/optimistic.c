 ...
 *
 * optimistic.c is part of Microbench
 * 
...
/*
 * Function optimistic_search corresponds to the findNode method of the original paper.
 * A fast parameter has been added to speed-up the search so that the function quits as soon
 * as the searched element is found.
 */
...
/*
 * Function optimistic_find corresponds to the contains method of the original paper.
 * In contrast with the original version, it allocates and frees the memory at right places
 * to avoid the use of a stop-the-world garbage collector.
 * In addition, it calls the search with parameter fast enables for the sake of optimization.
 */
int optimistic_find(sl_intset_t *set, val_t val) { 
	...
	//succs = (sl_node_t **)xmalloc(levelmax * sizeof(sl_node_t *));
	//found = optimistic_search(set, val, NULL, succs, 1);
...
/*
 * Function unlock_levels is an helper function for the insert and delete functions.
 */ 
...
int optimistic_insert(sl_intset_t *set, val_t val) {
	...
	while (1) {
		...
		for (i = 0; valid && (i < toplevel); i++) {
			...
			//valid = ((!(volatile int*) &pred->marked) && (!(volatile int*) &succ->marked) && 
			//				 (pred->next[i] == (volatile sl_node_t*) succ));
			
			valid = (!pred->marked && !succ->marked && 
							 ((volatile sl_node_t*) pred->next[i] == (volatile sl_node_t*) succ));
			//valid = ((!(volatile)pred->marked) && (!succ->marked) && 
				//			 (pred->next[i] == (volatile sl_node_t*) succ));
			
			/*if (cpt > 1000) {
					if (pred->marked) 
						printf("insert: pred is marked %d\n", valid);
					if (succ->marked) printf("insert: succ is marked\n");
					if (pred->next[i] != succ) printf("insert: %d not longer next of pred %d at level %d\n", (int) succ, (int) pred->next[i], i);
					cpt = 0;
			}*/
 ...
 * Here we avoid the fast search parameter as the comparison is faster in C than calling 
 * the Java compareTo method of the Comparable interface (cf. p132 of SIROCCO'07 proceedings).
 */
	...
	while(1) {
		...
		if (is_marked || (found != -1 && ok_to_delete(succs[found], found))) {		
			...
			for (i = 0; valid && (i < toplevel); i++) {
				...
				//valid = ((!pred->marked) && ((volatile sl_node_t*) pred->next[i] == (volatile sl_node_t*) succ));
				valid = (!pred->marked && ((volatile sl_node_t*) pred->next[i] == (volatile sl_node_t*)succ));
				
				/*if (cpt > 1000) {
						//if (pred->marked) 
					printf("remove: pred is marked\n");
						if (pred->next[i] != succ) 
							printf("remove: not longer next of pred at level %d\n",i);
						cpt = 0;
				}			*/
			...
			free(preds);
			free(succs);
			/* Freeing the freshly removed node */
			//sl_delete_node(node_todel);
			return 1;
		} else {
			/* Freeing the previously allocated memory */
			free(preds);
