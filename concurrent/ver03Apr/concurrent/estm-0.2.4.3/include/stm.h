 ...
 *   abort and the application should explicitly check its status.  If
 ...
 */
//void stm_elastic_start(TXPARAMS sigjmp_buf *env, stm_tx_attr_t *attr);
 ...
 *   null, the transaction uses default attributes.
 */
//void stm_normal_start(TXPARAMS sigjmp_buf *env, stm_tx_attr_t *attr);
	
void stm_start(TXPARAMS sigjmp_buf *env, stm_tx_attr_t *attr, int type);	
	
/**
 * Try to commit an elastic transaction.  If successful, the function 
 ...
 */
//int stm_elastic_commit(TXPARAM);

/**
 * Try to commit a normal transaction.  If successful, the function 
 * returns 1. Otherwise, execution continues at the point specified by 
 * the environment passed as parameter to stm_start() (for the outermost
 * transaction upon nesting).  If the environment was null, the function
 * returns 0 if commit is unsuccessful.
 */
//int stm_normal_commit(TXPARAM);
 ...
 */
//stm_word_t stm_elastic_load(TXPARAMS volatile stm_word_t *addr);
	
/**
 * Independent transactional load.  Read the specified memory location 
 * in the context of the current transaction and return its value.  
 * This operation is simply an atomic load of a value and its associated 
 * version that does not depend on any other operations.
 *
 * @param addr
 *   Address of the memory location.
 * @return
 *   Value read from the specified address.
 */
//stm_word_t stm_independent_load(TXPARAMS volatile stm_word_t *addr);
 ...
 *   Value read from the specified address.
 */
//stm_word_t stm_normal_load(TXPARAMS volatile stm_word_t *addr);
 ...
 *   Value to be written.
 * @return
 *   True (non-zero) if the operation succeeds.
 */
//int stm_elastic_store(volatile stm_word_t *addr, stm_word_t value);
	
//void stm_normal_store(volatile stm_word_t *addr, stm_word_t value);
...
/**
 * Elastic transactional store.  Write a value to the specified memory location
 ...
 * to indicate the bits of the words that must be updated.  The 
 * transaction may abort while writing to the memory location.
 *
 * @param addr
 *   Address of the memory location.
 * @param value
...
/**
 * Normal transactional store.  Write a value to the specified memory location
 * in the context of the current transaction.  The value may be smaller
 * than a word on the target architecture, in which case a mask is used
 * to indicate the bits of the words that must be updated.  The 
 * transaction may abort while writing to the memory location.
