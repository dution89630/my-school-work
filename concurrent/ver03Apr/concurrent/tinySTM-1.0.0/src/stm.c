...
#endif
#if DESIGN == WRITE_BACK_CTL
  printf("write_back_ctl stm\n");
#endif
#if DESIGN == WRITE_THROUGH
  printf("write_through stm\n");
#endif
...
stm_word_t stm_unit_load(volatile stm_word_t *addr, stm_word_t *timestamp)
{
  ...
  //w_entry_t *w;
  //stm_word_t t;
  /* struct timeval start, finish, res; */
#if DESIGN == WRITE_BACK_ETL
  w_entry_t *w;
#endif /* DESIGN == WRITE_BACK_ETL */
#if DESIGN == WRITE_BACK_CTL
 ...
 restart_no_load:
  if (LOCK_GET_OWNED(l)) {
    ...
    w = (w_entry_t *)LOCK_GET_ADDR(l);
    t = w->tx->status;
    if(GET_STATUS(t) == TX_COMMITTING) {
      //do nothing?
      //printf("is committing\n");
    }
    else if (IS_ACTIVE(t) && !tx->irrevocable)
	{
	  /* Read old version */
	  //version = ATOMIC_LOAD(&w->version);
	  /* Read data */
	  ...
	  if(GET_STATUS(t) == TX_COMMITTING || !IS_ACTIVE(t)) {
	    //tx->locked_reads_failed++;
	    //printf("is committing again\n");
	    goto restart;
	  }
	  tx->locked_reads_ok++;
	  //printf("got it ok\n");
