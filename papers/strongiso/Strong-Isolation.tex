...
\line{A07} \>\>\> $\mathit{value} \gets \mathit{tmp.last}$ \\
\line{A08} \>\> {\bf else} $\mathit{value} \gets \mathit{tmp.value}$ {\bf end if} \\
\line{A09} \> {\bf else} $\mathit{value} \gets \mathit{tmp.value}$ {\bf end if} \\
...
\line{C04} \>\> {\bf then} {\bf if} $\mathit{tmp.time} >= \mathit{rv}$ {\bf then} \\
\> \% Do validation to prevent abort due to a non-transactional write \\
\line{C05} \>\>\> {\bf if} this is an update transaction {\bf then} ${\sf validate\_by\_value}$ () \\
...
\line{C10} \>\>\> {\bf if} $\mathit{status} =$ LIVE {\bf then} ${\sf abort}()$  {\bf else} $\mathit{value} \gets \mathit{tmp.last}$ {\bf end if} \\
\line{C11} \>\> {\bf else} $\mathit{value} \gets \mathit{tmp.value}$ {\bf end if}; \\
...
\line{START3} \> $\mathit{status} \gets $LIVE; \ $\mathit{ws} \gets \emptyset$; $\mathit{rs} \gets \emptyset$; $\mathit{ntrs} \gets \emptyset$ \\
%\line{DA03} \> more??? \\
{\bf end operation}. \\
\\
...
\line{DA05} \>\>\> {\bf if} $\mathit{status} =$ LIVE {\bf then} ${\sf abort}()$  {\bf else} $\mathit{item.last} \gets \mathit{tmp.last}$ {\bf end if} \\
\line{DA06} \>\> {\bf else} $\mathit{item.last} \gets \mathit{tmp.value}$ {\bf end if}; \\
...
\line{H05} \>\>\> {\bf if} $\mathit{tmp.status} =$ LIVE $\cap \mathit{item} \not \in \mathit{ws}$
     {\bf then} ${\sf abort}()$ {\bf end if}; \\
\line{H06} \>\>\> $\mathit{new\_value} \gets \mathit{tmp.last}$ \\
\line{H07} \>\> {\bf else} $\mathit{new\_value} \gets \mathit{tmp.value}$ {\bf end if}; \\
...
This algorithm is shown in figure \ref{fig:ntops2}-\ref{fig:tbc2}
...
\line{A13} \>\>\> $\mathit{value} \gets \mathit{tmp.last}$ \\
\line{A13} \>\> {\bf else} $\mathit{value} \gets \mathit{tmp.value}$ \\
\line{A07} \>\> {\bf end if} \\
\line{A07} \> {\bf else} $\mathit{value} \gets \mathit{tmp}$ {\bf end if} \\
\line{C23} \> ${\sf return}$ $(\mathit{value})$ \\
{\bf end operation}. \\
\\
{\bf operation}  ${\sf non\_transactional\_write}(\mathit{addr, value})$ {\bf is}\\
...
\line{C06} \>\> {\bf if} $\mathit{status} =$ LIVE {\bf then} ${\sf abort}$() {\bf end if} \\
\line{C25} \>\> {\bf if} $(\mathit{tmp.time} > \mathit{rv})$ {\bf then} ${\sf abort}$() {\bf end if}; \\
\line{C06} \>\> {\bf if} ($\mathit{status} =$ COMMITTED {\bf then}  \\
\line{C25} \>\>\> $\mathit{value} \gets \mathit{tmp.val}$ \\
\line{C06} \>\> {\bf else} $\mathit{value} \gets \mathit{tmp.last}$ {\bf end if} \\
\line{C06} \> {\bf else} \\
\> \% Do validation to prevent abort due to a non-transactional write \\
\line{C16} \>\> $\mathit{rv} \gets {\sf validate\_by\_value}()$ {\bf end if}; \\
\line{C25} \>\> $\mathit{value} \gets \mathit{tmp}$ \\
\line{C16} \> {\bf end if} \\
\line{C20} \> {\bf if} this is an update transaction {\bf then} add $\mathit{value}$ to $\mathit{rs}$ {\bf end if}; \\
\line{C23} \> ${\sf return}$ $(\mathit{value})$ \\
...
\line{C05} \>\> {\bf if} 
   $\mathit{tmp}$ is of type $T$ {\bf then} \\
\line{C05} \>\>\> {\bf if} ($\mathit{status} \gets \mathit{tmp.status}$)$ =$ COMMITTED {\bf then} $\mathit{item.last} \gets \mathit{tmp.value}$ \\
\line{C06} \>\>\> {\bf else if} $\mathit{status} =$ ABORTED {\bf then} $\mathit{item.last} \gets \mathit{tmp.last}$ \\
\line{C05} \>\>\> {\bf else} ${\sf abort}()$ {\bf end if} \\
\line{C11} \>\> {\bf else} $\mathit{item.last} \gets \mathit{tmp}$ {\bf end if}; \\


\line{DA07} \>\> {\bf if} $(\neg {\sf \mathit{C\&S}}(\mathit{item.addr, tmp, item}))$  \\
\line{DA08} \>\>\> {\bf then} ${\sf abort}()$ \\
\line{DA10} \>\> {\bf end if} \\
...
{\bf operation}  ${\sf non\_transactional\_read}(\mathit{addr})$ {\bf is}\\
\line{A01} \> $\mathit{lock} \gets {\sf load\_lock}(\mathit{addr})$; \\ 
\line{A01} \> $\mathit{value} \gets {\sf load}(\mathit{addr})$; \\ 
\line{A02} \> {\bf if} ($~\mathit{lock}$ is locked  $\wedge \mathit{tmp.status} = $ COMMITTED $\wedge \mathit{addr} \in \mathit{lock.ws}$) {\bf then} \\
\line{A04} \>\> {\bf then} $\mathit{value} \gets \mathit{item.value}$ from $\mathit{addr}$ in $\mathit{lock.ws}$ \\
\line{A07} \> {\bf end if} \\
...
\\
{\bf operation}  ${\sf non\_transactional\_write}(\mathit{addr, value})$ {\bf is}\\
\line{B05} \> Perform a transactional begin/write/commit operation \\
{\bf end operation}.
