...
Specifically we will show that every transaction (aborted or commited) as well
...
Note that we do not use an STM specfic consistency criterion
...
The post-condition is that the value of $\mathit{next\_write.value}$ is returned by 
read operations to $\mathit{addr}$ (transactional and non-transactional) that are linearized afterwards.
For either type of read operation their linearization point is when the the value of $\mathit{addr}$ is read,
therefore any read operation linearized after an NT-write (and before another operation writes to $\mathit{addr}$
will read a pointer the pointer written by the NT-write.
Now, looking at the code of the read operations it is easy to see that when an object of type NT is read from
$\mathit{addr}$ the 
\end{proof}
