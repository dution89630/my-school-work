...
\line{DA12} \> $\mathit{time} \gets {\sf increment\&fetch}(\mathit{GVC})$; ${\sf validate\_by\_value}()$; \\
...
In this case, if the object dereferenced on line \ref{C01} is an NT record then we must have $\mathit{tmp.time} < \mathit{rv}$
...
If the object dereferenced on line \ref{C01} is a T record then we must have $\mathit{tmp.time} \leq \mathit{rv}$ and 
