...
The post-condition is that the value of $\ms{next\_write.value}$ is returned by 
read operations to $\ms{addr}$ (transactional and non-transactional) that occur after the linearization point.
