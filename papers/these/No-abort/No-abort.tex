...
to ensure the amount of safety a programmer would expect from an atomic block without having
...
Opacity and other consistency criterion are generally considered to be safety properties.
...
What such consistency criterion do not consider (among other things) is how often transactions commit.
...
one person on this project might have an approaching deadline and he might not want to wait on someone across the
...
This property ensure that no thread is ever stuck waiting for another thread, but only guarantees
...
transactional memory has been performed in \cite{BGK12}.
...
several solutions have been proposed witch each taking different approaches
to progress.
...
of an transactional memory implementation, but this thesis takes the view
that the primary goal of transactional memory is ease-of-use and having programmers
...
with the purpose of tracking the  locations the STM  has read so far as well as those
