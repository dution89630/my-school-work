...
processors or machines, some might be sleeping, some might execute slower then others, the connection between
...
that every process that executes alone for long enough
...
\paragraph{Contention Management}
...
other processes then the one that issued the transaction, thus
...
Still given this, to follow our high level Definition \ref{def:trans-def} of transactions we must take into account all parts
of the system, as such we need to consider how these non-reversible operations fit into transactions
in our definition.
Since this problem is not focused on in this thesis,
future research should consider these operations and the surrounding questions left unanswered.
For example, when considering progress, an important question for
...
As suggested in the introduction and described specifically later in this chapter,
