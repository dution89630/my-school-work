...
%(Due to page limitation, see also appendix \ref{annex:related-work})
...
%(e.g., when  those are  atomic read/write objects).  
In that case
...
The proposed construction can be seen as a 
{\it universal construction} for transaction-based concurrent programs. 
%Every  execution  (under the  control  of  this  construction) of  such  a
%concurrent    program is linearizable.
%
%and terminates if the multi-process it simulates is terminating.  
%
...
Section \ref{sec:related-work} presents related works.
Section \ref{sec:models}  presents the model offered to  the programmer and
the underlying multiprocessor model on which the STM system is built. 
Section \ref{sec:construction} presents the universal construction 
(STM system) for transaction-based concurrent programs.   
Section  \ref{sec:proof} addresses  correctness.  
%Section \ref{sec:discussion} discusses additional features 
%of the proposed construction (those are mainly related to the net 
%effect of  asynchrony and failures, etc.).
Section \ref{sec:bounded-tries}
shows that the number of times a transaction can be executed before being committed is bounded.
Finally, Section \ref{sec:conclusion} concludes the paper
by discussing  additional features of the proposed construction.
%(those are mainly related to the net effect of asynchrony and failures, etc.).
...
%\paragraph{Helping mechanism: the array $\mathit{\it LAST\_CMT}[1..m,1..n]$}
\noindent
...
%\paragraph{Helping mechanism: the array $\mathit{STATE}[1..n]$}
\noindent
...
\line{CA08} \>\>\>\>\>  {\bf if} \= ($k1\_counter \geq  K1$) {\bf then} \\


\line{CA09} \>\>\>\>\>\>  {\bf let} $j~=~my\_next\_proc$;\\ 


\line{CA10} \>\>\>\>\>\> {\bf if} \= 
       $(state[j].help\_date = + \infty)$ {\bf then}\\
...
% and $P_x$'s boolean   variable  $\mathit{after\_my\_last\_cmt}$.  
...
%Due to page limitation, the proof is given in Appendix \ref{appendix:proof}. 
...
A processor $P_y$ commits a transaction $T$ issued by process $p_i$ 
(owned by processor  $P_x$)  when, after it has  simulated the execution of
$T$ at lines  \ref{BA13}-\ref{BA17}, it executes successfully 
the Compare\&Swap statement at  line \ref{BA28} of 
Figure~\ref{fig:main-construction}. The execution context in which $P_y$
has executed  $T$, denoted $context(T)$ is defined by the  pair 
$\langle i\_local\_state_T, current_T \rangle$ where 
$i\_local\_state_T$ is the value previously read by $P_y$ from  
$\mathit{STATE}[i].localstate$ at line \ref{BA03} 
and  $current_T$ is the last value of $current$ read by $P_y$ 
before exiting the while loop of lines \ref{BA06}-\ref{BA11}. 
As we have seen, the values  of the $t$-objects read by $T$ are 
obtained by scanning downwards the descriptor list from  $current_T$




%-------------------------------------------------------------------------
\subsection{Base lemmas}

Let  us consider  a  processor  $P_y$ that   simulates the  execution  of a
transaction $T$ issued by a process $p_i$ (lines   \ref{BA13}-\ref{BA17}). 
The execution context in which $P_y$
executes  $T$, denoted $context(T)$ is defined by the  pair 
$\langle i\_local\_state_T, current_T \rangle$ where 
$i\_local\_state_T$ is the value previously read by $P_y$ from  
$\mathit{STATE}[i].localstate$ (line \ref{BA03}) 
and  $current_T$ is the last value of the pointer $current$ read by $P_y$ 
before exiting the while loop of lines \ref{BA06}-\ref{BA11}. 
As we have seen, the values  of the $t$-objects read by $T$ are 
obtained by scanning downwards the descriptor list from  $current_T$


$context(T)$ is  {\it valid} if $i\_local\_state_T$ and the values 
of the $t$-objects  read by $T$ could have obtained in a sequential 
execution of 
...
%---------------------------
\begin{lemma}
\label{lemma:valid-context}
Any transaction $T$ issued by a process is executed in a valid context.
\end{lemma}

\begin{proofL}

\renewcommand{\toto}{lemma:valid-context}
\end{proofL}
%---------------------------
} %%%%%%%%%%% end of \Xomit{%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
%\textcolor{red}{I  feel like this  might be  more clear  if we  broke this
%lemma into 3 lemmas and we each proved them separately: 
%1. The transaction invocations issued by the processes are linearizable. 
%2. Each transaction  invocation executes in a consistent  global and local
%state. 
%3. Each block of NT-code executes in a valid local state.}
%
\end{lemma}
...
from (or the start of $T$ if it happens later),
%at its least read of a $t$-object,
which makes it linearizable.  
\end{itemize}
%\vspace{-0.4cm}
...
$O(1)$.
%Also notice that  the committal of $T1$ makes no changes  to the number of
%uncommitted transactions in the $STATE$ array with their help flags set to
%$true$.
%\vspace{-0.2cm}
\item
...
%Then eventually line  \ref{30} will be executed and  the $LAST_COMT$ array
%will be  updated so $T1$  will no longer  be selected to execute  in lines
%\ref{BA02} and \ref{BA03} effect
...
%
%\vspace{-0.2cm}
%\item  Interestingly, read-only transactions are never aborted 
%and, consequently, do not require help.
%
%\vspace{-0.2cm}
\item  
...
%
%\vspace{-0.2cm}
%\item Think to the cost (according to number of transactions, 
%number of tries, number of processors, etc.)?
%
\end{itemize}
...
% The authors want to thank Vincent Gramoli and Pascal Felber 
% for interesting discussions on STM systems. 
% %
% This 
% research is part  of the  Marie Curie ITN project TRANSFORM
% funded by the European Union  FP7 Program (grant 238639).
