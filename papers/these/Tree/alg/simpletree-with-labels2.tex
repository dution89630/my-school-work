...
aaaaa\=aa\=aaaaa\=aa\=\kill %~\\
 ...
 \bf{State of node $n$} \\
	...
	\line{TBA01} \>\> $\ms{left-h}, \ms{right-h} \in \mathbb{N}$, local height of left/right child, initially 0 \\
...
\bf{operation} $\act{contains}(k)_p$ \\
		...
		\line{TBA01} \> $\sf{return}(\ms{result})$. \\%\label{sline:cont-return} \\
...
\bf{operation} $\act{insert}(k,v)_p$ \\
		...
		\line{sline:new-node} \>\> \bf{else} \sf{allocate a new node} \\%\label{sline:new-node} \\
