  ...
  \begin{algorithmic}[1]
   ...
   {\size 
   		\Part{Additional fields of IndexItem $item$}{
			...
			\State \T $\ms{next}$, pointer to the next IndexItem
			\State \T\T in the SkipList
			\State \T $\ms{down}$, pointer to the IndexItem one
			\State \T\T level below in the SkipList
			\State \T $\ms{node}$, pointer a node in the list at
			\State \T\T the bottom of the SkipList
   		}\EndPart
		...
		\Part{Additional fields of node $n$}{
			...
			\State \T $\ms{prev}$, pointer to the previous node
			  \State \T\T in the list
			\State \T $\ms{level}$, integer indicating the level of
			\State\T\T the node, initialized to $0$
			\State \T\T 
   		}\EndPart
%		\Statex   	
%		
...
%   		}\EndPart
		...
		\Part{State of structure $s$}{
			\State $\ms{top}$, pointer to the first and highest
			\State \T level IndexItem in the SkipList
			\State $\ms{first}$, array of pointers to the first item
			\State \T of each level in the SkipList
			\State $\ms{bottom-index}$ integer indicating the
			\State \T level of the bottom IndexItem
