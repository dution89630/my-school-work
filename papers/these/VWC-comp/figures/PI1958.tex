%% Squelette de document LaTeX pour Publi interne UMR
%% Philippe Louarn - Inria Rennes - 1993/2008

\documentclass{article}

%\usepackage{french} %%% Ôter le commantaire '%' en début de ligne si Langue= F

\usepackage{umr-V2}      %%% Rapport de recherche Inria %%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\RRtitle{Les lectures invisibles, la coh\'erence des mondes virtuels et
la permissivit\'e sont compatibles}


\RRetitle{Read invisibility,
...
%
\RRauthor{
Tyler Crain\thanks{Projet ASAP: \'equipe commune avec l'INRIA, le CNRS, l'universit\'e Rennes 1 et l'INSA de Rennes},
Damien Imbs\thanks{Projet ASAP: \'equipe commune avec l'INRIA, le CNRS, l'universit\'e Rennes 1 et l'INSA de Rennes},
Michel  Raynal\thanks{Projet ASAP: \'equipe commune avec l'INRIA, le CNRS, l'universit\'e Rennes 1 et l'INSA de Rennes}
\\
{\it tyler.crain@irisa.fr, damien.imbs@irisa.fr, raynal@irisa.fr}
}
%       

\authorhead{T. Crain, D. Imbs  \&  M. Raynal}
\titlehead{Read invisibility,
           virtual world consistency and  permissiveness
           are compatible}

\RRdate{novembre 2010}  %% date de publication %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\RRresume{Ce rapport montre que les lectures invisibles, la coh\'erence des mondes virtuels et
la permissivit\'e sont compatibles. Il pr\'esente un algorithme qui r\'eunit ces conditions.
 }

\RRmotcle{M\'emoire partag\'ee, Commit/abort, Objet concurrent,  
Condition de coh\'erence, Opacit\'e,  Permissivit\'e, Serialisabilit\'e,   
M\'emoire transactionnelle,  Transaction, Coh\'erence des mondes virtuels. }

\RRabstract{The aim of a Software Transactional Memory (STM) is to discharge the 
...
Both conditions ensures that any transaction (be it a committed 
...
\RRkeyword{Asynchronous shared memory system, Commit/abort, Concurrent object,  
...
%%%%%%%%%%%%
\makeRR            %% Activation de tout ce qu'on vient de déclarer %%%%%%%
%%%%%%%%%%%%
...
The  aim of  an STM   system   is to  simplify the design and the writing 
of  concurrent  programs by discharging the programmer from the explicit
management of synchronization entailed by concurrent accesses to shared 
objects. This means that, when faced to synchronization,  a programmer has 
...
transactions is serializability \cite{P79} (sometimes strengthened in ``strict
serializability'',   as  implemented  when   using   the   2-phase  locking
mechanism). The serializability  consistency criterion 
...
occurs,  the transaction  throws  an  exception  that  has to  be
...
requires  that  no transaction reads  values from an inconsistent global 
...
previously) reads values that are  consistent  with respect to its causal  
past  only. 
...
(hence the name {\it virtual world} consistency).  This  consistency 
...
opacity,  one of  the main advantages  of virtual world  consistency lies 
...
incorrectly despite  concurrency, it can only  be aborted. This  is a first
class requirement for transactional memories. 



%----------------------------------------------------------------------
\subsection{Desirable properties for STM systems}
...
This paper is on permissive STM systems with invisible reads. 
It has several contributions. 
\begin{itemize}
%\vspace{-0.2cm}
\item  
It  first shows that an STM system that satisfies read invisibility and 
opacity  cannot be permissive. 

%\vspace{-0.2cm}
\item 
The paper then presents an STM system (called IR\_VWC\_P)  that satisfies 
read invisibility,  virtual world consistency and  permissiveness. 
%
The STM IR\_VWC\_P protocol  presents additional noteworthy properties. 
\begin{itemize}
%\vspace{-0.2cm}
\item  
It uses only  base read/write operations and locks,  each associated with a
shared object.   Moreover,  a lock is used at most once by a transaction 
at the end of a transaction (when it executes an operation 
called    ${\sf try\_to\_commit()}$). 
%\vspace{-0.1cm}
\item  
%Except for the access to a  global shared clock 
it   satisfies the disjoint access parallelism property.  
%\vspace{-0.1cm}
\item  
It allows for {\it fast} read operations. 
More precisely, the first read of an object $X$ by a transaction $T$ 
can cost up to $O(|lrs_T|)$ shared memory accesses (where $lrs_T$ is the 
current read  set of $T$) in the worst case and  $O(1)$ shared memory accesses 
in favorable cases. The cost of all the following reads of $X$ by 
$T$ is $O(1)$.  
\end{itemize}
\end{itemize}


The paper is made up of \ref{sec:conclusion} sections. 
Section \ref{sec:model-and-conditions} presents the computation model. 
Section  \ref{sec:opacity-and-VWC} presents the  opacity and  virtual world
consistency conditions. 
Section \ref{sec:opacity-impos} shows that opacity, read invisibility 
and permissiveness are incompatible. 
The IR\_VWC\_P protocol is then described incrementally. 
Section \ref{sec:algorithm-V1}  presents first a base  version 
that satisfies the VWC property and  invisibility of  read operations. 
This protocol is proved in Section \ref{sec:algorithm-V1-proof}
and improved in Section \ref{sec:improving-base-protocol}
(these improvements concern additional features such as 
garbage  collection  of  useless  cells   and   fast   read    operations).
Finally,   Section     \ref{sec:algorithm-V2}  enriches  it   to  obtain  a
permissive protocol.  Section  \ref{sec:conclusion} concludes the paper. 
...
A transaction  can read or write any shared object. 
...
Transaction are assumed to b dynamically defined. The important point is here 
...
\begin{figure}[ht]
\centering
\scalebox{0.30}{\input{virtual-world-consistency.pdf_t}}
...
the ones with a grey square have aborted. From a dependency point of view, 
...
%=========================================================================
\section{Invisible reads, opacity and permissiveness are incompatible} 
...
has to de decided, $T_3$ is not aware of the existence of $T_1$.
...
\begin{figure}[htb]
\centering{
  \scalebox{0.45}{\input{fig-opacity-impossibility.pdf_t}}
...
\end{itemize}
If follows from the previous item that $T_1 \rightarrow_{PO}   T_1$. 
...
\paragraph{Remarks}
The previous proof shows that opacity is a too strong consistency condition 
when one wants  both  read invisibility and permissiveness. 
Differently, when considering the previous execution, 
the  virtual  world consistency protocol IR\_VWC\_P presented in this paper  
will abort transaction $T_1$. 
It is easy  to see that the  corresponding weak transaction history 
...
%=========================================================================
\section{Step 1: Ensuring virtual world consistency with read invisibility}
\label{sec:algorithm-V1}

As announced  in the Introduction,  the protocol IR\_VWC\_P  is built  is two
...
protocol to obtain  permissiveness.
...
%-------------------------------------------------------------------------
\subsection{Base objects, STM interface, incremental reads and 
deferred updates}
...
%-------------------------------------------------------------------------
\subsection{The underlying data structures}
...
\begin{figure}[htb]
\centering{
  \scalebox{0.45}{\input{fig-implementing-register-X.pdf_t}}
...
%
%--------------------------------------------------------------------------
\subsection{The ${\sf read}_T()$ and  ${\sf write}_T()$  operations}
...
%--------------------------------------------------------------------------
\subsection{The ${\sf try\_to\_commit}_T()$ operation}
...
%===========================================================================
\section{Proof of the algorithm for VWC and read invisibility}
...
\subsection{Proof that $\mathcal{C}$ is serializable}
...
%------------------------------------------------------------------
\subsection{VWC and read invisibility}
\begin{theorem}
\label{theo:vwc}
...
This section considers three improvements of the base protocol 
previously  presented and proved. Those concern the following points:
how the useless cells  can be collected, how read operations can 
be made fast  and how serializability can be replaced by linearizability. 
...
%===========================================================================
\section{Step 2: adding probabilistic permissiveness to the  protocol}
...
\subsection{The IR\_VWC\_P protocol}
...
\begin{figure}[ht]
\centering
\scalebox{0.35}{\input{fig-intervals.pdf_t}}
...
(line~\ref{BA20}.H; see Appendix~\ref{sec:appendix-forbid} for an example). 
...
An example is given in Appendix \ref{sec:appendix-example-for-ct}. 
...
Figure \ref{fig:ttc-V22-protocol}, is permissive with respect to 
...
This paper has investigated the relation linking read invisibility, 
...
single-version. The STM   protocol presented in this paper satisfies 
...
(ICDCN'11)}, Springer-Verlag, LNCS \#abcd, pp.~aaa-bbb, 2011.
...
%=================================================================
\appendix  
\section{More on fast read operations}
...
no realtime-compliant legal linear extension with previously committed 
...
\begin{figure}[ht]
\centering
\scalebox{0.35}{\input{fig-opac-impos-fastreads.pdf_t}}
...
%=================================================================
\section{Making read operations invisible at commit time}
...
%=================================================================
\section{Substraction on sets of  intervals (line \ref{BA20}.H of 
...
\begin{figure}[ht]
\centering
\scalebox{0.35}{\input{fig-intervals-forbidden.pdf_t}}
...
%=================================================================
\section{About the predicate of line \ref{BA24}.A 
...
\begin{figure}[ht]
\centering
\scalebox{0.35}{\input{fig-constraint-commit-time.pdf_t}}
