...
The most imporant goal of the concept of STM is to make concurrent programming
easier and more accessable to any programmer.
...
realized. 
(Unfortunately, in reality, it is actually not so simple for the programmer, this is discussed in
...
The most obvious is that there are different data stuctures that can be used to
provide a map abstraction, wich examples such as a skip-list, binary tree, or hash table.
Chosing one of these may impact the applications performance or provide additional
functionality in additon to that provided by the map abstraction.
Even when considering a single data sturcture there are different properties a specific implementation might ensure.
...
Beneth these atomic blocks lies the STM implementation which has many different aspects.
Since the introduction of transactional memory in 1993 \ref{} dozens of different properties have emerged
as well as many different STM algorithms.
With each of these ensuring a greater or fewer number of these properties and more or less
concerned with performance.
(In the related work section (section \ref{}) we give an overview of many different properties and
implementations of STM)

In an ideal world there would exist a \emph{``perfect''} STM algorithm that ensures all desirable properties
without making any sacrifies.
Unfortunately this algorithm has not yet been discovered (if it is even possible).
In fact many of these desierable properties have been little more then introduced and many of their implications
on how they affect STM algorithms or how they interact with eachother has yet to be explored.
Motivated by this, this chapter examines two desiralbe properties that are concerned with performance,
...
the study of attitional properties.
...
If you have not noticed already, a reocurring theme throughout this document is that
the most important goal of transactional memory is ease of use,
and is the subject of this section.
We alreay have our syntax defined for the programmer ($atomic\{ \dots \})$) and
a basic idea of what this means, ``the code in the atomic block will appear as if
it has been executeded instantaneously'', yet we need to precisely define what this
means for an STM algorithm.
At the heart of this we have consistency criterions.
These criterions precisely define the semantics of a transaction and guide the creation
...
In this section we give a overview of two well known consitency criterion previously
