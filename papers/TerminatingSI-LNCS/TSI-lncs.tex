...
(b) any non-transactional read or write operation always terminates (there no notion of commit\slash abort associated with them). 
\keywords{Transactional Memory, Strong Isolation, Atomicity} %, TL2, Terminating Operation, Opacity, Non-transactional Operation, Consistency}
\end{abstract}
...
of shared memory on and, on the other hand, considering TM as an additional way of achieving synchronization, 
...
This definition implies  what   is  referred  to  as  
{\it containment}  and
{\it non-interference} \cite{blundell06} be satisfied.  
...
\begin{figure*}[ht]
\centerline{
%     \mbox{\includegraphics[width=0.4\textwidth]{imgs/non_containment.eps}}     
		\mbox{\epsfig{file=imgs/non_containment.eps, width=0.5\textwidth, clip=}}
%    \mbox{\includegraphics[width=0.4\textwidth]{imgs/interference.eps}}
		\mbox{\epsfig{file=imgs/interference.eps, width=0.5\textwidth, clip=}}
}
...
transaction follow the original TL2 algorithm rather closely (
cheap read only transactions, commit-time locking, write-back), 
...
The memory setup  is illustrated in Figure  \ref{fig:mem_setup}.
...
\begin{figure*}[ht]
\centerline{
    \mbox{\includegraphics[width=0.6\textwidth]{imgs/mem_setup_single.eps}}
}
...
Also garbage collection will be required for old T and NT-records, here we assume
...
shown in Figure \ref{fig:ntops} must be used when a shared variable is accessed
...
so that linearizability (containment specifically) is not violated.
...
and ${\sf try\_to\_commit()}$, presented in Figure \ref{fig:tbc}. 
