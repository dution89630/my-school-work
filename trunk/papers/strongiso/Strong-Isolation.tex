...
\line{DA08} \>\> {\bf if} $(\neg {\sf \mathit{C\&S}}(\mathit{addr, tmp, item}))$  \\
...
\line{MA02} \> {\bf if} $( ~\mathit{tmp}$ is of type T ) {\bf then} \\
\line{MA03}  \>\>  {\bf if}  $\mathit{tmp.status} = $ LIVE \\
\line{MA04} \>\>\> {\bf then} \=${\sf \mathit{C\&S}}$($tmp.status$, LIVE, ABORTED) \\
\line{MA05} \>\> {\bf end if} \\
\line{MA06} \>\> {\bf if} $tmp.status = $ ABORTED {\bf then} \\
\line{MA07} \>\>\> $\mathit{value} \gets \mathit{tmp.last}$ \\
\line{MA08} \>\> {\bf else} $\mathit{value} \gets \mathit{tmp.value}$ \\
\line{MA09} \>\> {\bf end if} \\
\line{MA10} \> {\bf else} $\mathit{value} \gets \mathit{tmp}$ {\bf end if} \\
...
\line{MC04} \>\> {\bf if} $\mathit{status} =$ LIVE {\bf then} ${\sf abort}$() {\bf end if} \\
\line{MC05} \>\> {\bf if} $(\mathit{tmp.time} > \mathit{rv})$ {\bf then} ${\sf abort}$() {\bf end if}; \\
\line{MC06} \>\> {\bf if} ($\mathit{status} =$ COMMITTED {\bf then}  \\
\line{MC07} \>\>\> $\mathit{value} \gets \mathit{tmp.val}$ \\
\line{MC08} \>\> {\bf else} $\mathit{value} \gets \mathit{tmp.last}$ {\bf end if} \\
\line{MC09} \> {\bf else} \\
\> \% Do validation to prevent abort due to a non-transactional write \\
\line{MC10} \>\> $\mathit{rv} \gets {\sf validate\_by\_value}()$ {\bf end if}; \\
...
\line{MDA04} \>\> {\bf if} 
   $\mathit{tmp}$ is of type $T$ {\bf then} \\
\line{MDA05} \>\>\> {\bf if} ($\mathit{status} \gets \mathit{tmp.status}$)$ =$ COMMITTED {\bf then} $\mathit{item.last} \gets \mathit{tmp.value}$ \\
\line{MDA06} \>\>\> {\bf else if} $\mathit{status} =$ ABORTED {\bf then} $\mathit{item.last} \gets \mathit{tmp.last}$ \\
\line{MDA07} \>\>\> {\bf else} ${\sf abort}()$ {\bf end if} \\
\line{MDA08} \>\> {\bf else} $\mathit{item.last} \gets \mathit{tmp}$ {\bf end if}; \\
...
\line{N03} \> {\bf if} ($~\mathit{lock}$ is locked  $\wedge \mathit{tmp.status} = $ COMMITTED $\wedge \mathit{addr} \in \mathit{lock.ws}$) {\bf then} \\
