...
To further complicate things for the people implementing the actual datat sturcutures,
...
Since the introduction of transactional memory in 1993 \cite{} dozens of different properties have emerged
...
otherwise be safely commmitted, i.e. the protcols are not permissive.
...
In this section we give a overview of two well known consitency criterion
...
serializability''.
Strict serializability has the additional constranst that the equivalent sequentail history
...
mechanism.
Strict serializability is often refered to as linearizability \cite{}  when considering the operations
...
(no matter if it is eventually commited or not).
...
In this section we examine the some of the implcations of these properties
on STM algorithms.

We start by proving that an STM portocal cannot implemnt invisible reads, opactiy,
and permissiveness at the same time.
This is importat because it shows that no matter how well we keep track of
the interatctions between transactions, an opaque STM portocol will have to abort
...
as the consistency condition no transactions need to be aborted unenessarily.
Importantly changing virtual world consictency for opacity makes no difference
...
!!!!!!!!!!!!!!!SHOULD ALSO TALK ABOUT PROB PERM FOR THE PROOF!!!!!!!!!!!
...
\anote{!!!!!!!!!NEED TO INTRO/UPDATE THIS SECTION!!!!!!}
...
The previous sections investigated the interatcions between a few different STM
properties and consistency conditions.
It showed that premissiveness, opacity, and invisible reads are imcompatable, but
...
practiacl STM algorithms, but
does not provide a complete examination of STM
as it leaves us with no practiacl implementations.

This section begins an investigaion towards this more practiacl study of algorithms.
...
figure \ref{fig:ttc-protocol} could be modified in order to make the read operations cheaper whithout impacting the correctness of
...
To conclude this chapter we will return to the idea that the main pourpose of
...
some consistency criteron.
This is important because these consitency criterion define the semantics
of a transaction to the programmer.
As long as the choosen consistency criterion is satisfied then we can start considering other
secondary, but still important things such as performance.
This is where different properties such as read invisiblity and permissiveness
come in, a protocol that chooses or not to implement such properties might impact
the performace of that protocol, but does not change the semantics of a transaction.
...
read invisibility.
Most importatnly, even though we have weakend the consistency criteron, the programmer
will see no difference in the semaintics of a transaction.
If virtual world consistency changed the way a programmer had to think about transactions versus opacity
then it would not be considered an appropriate consistency criteron for transactional memory.

The second contribution of this chapter introduces a relistic algorithm that satisfies
...
An important distinction of this work from most is that instead of looking at properties independtly,
it considers multiple properties at once.
More specifally it looks at the compaability of properties, how realistic protocols can be
desinged to satisfy them, and how trade-offs in these proterties can be made for efficiency.
Hopefully these contributions have convinced the reader that studing such combinations is important for understanding
transactional memory and will help inspire similar research on the many combinations
left unexamined.

As a final note on ease-of use, it should be said that the STM protocols introduced in this chapter are similar to
the majority of published work on transactional memory in that they concern
designing an efficient protcol satisfying without modifying the idea of a transaction.
What is important to realize is that all this work has no impact on how the programmer
understands the transaction, it puts ease-of-use first.
In the following chapters we will examine some of what we consider shortcomings
of the commonly used transactional model and how to improve them.
