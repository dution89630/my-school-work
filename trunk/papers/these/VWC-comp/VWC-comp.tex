...
better or wore depending on the workload.
...
As viewed, the recurring theme throughout this document is that
...
Before getting into the formal definitions of consistnecy condiditons the following section describes some additional properties
...
interestingly enough, this  STM protocol  has additional  noteworthy
features described in the following paragraphs. 
By satisfying these properties we then know that they are compatible
with permissiveness, virtual world consistency, and invisible reads.
It is important to realize that some of these properties are not necessarily
exclusive to STM systems and may be applicable to other concurrent algorithms,
or even algorithms in general.
Realizing this, there are countless properties that can be considered whose
compatibility can be then examined with other properties resulting in a huge
number of possibilities.
Given such a huge space of options to examine, people studying
TM are given the task of deciding which ones might be important and others
that might not be so much.
Unfortunately this is not always a straightforward task, especially given
that we do not know to what extent and for what problems transactional memory
might be used for in the future.
The following are some properties satisfied by the protocol which appear
...
compared to just using simple reads and writes.
...
The protocol described in this chapter uses only base read/write operations and a lock per object 
that is  used   at  commit  time   only.
...
It may seem like this is a straightforward property to ensure, but due to the
fact that the locations a transaction might access are not known until they actually happen,
it can be difficult to design safe disjoint access parallel STM protocols.
Still, this is a nice property for an STM system to implement for several reasons.
First is scalability, a program that is written so that each process does not share any
data with any other process could have its scalability limited by the sharing of data within
the STM implementation.
Second is simply efficiency, as shared data that is expensive accessed.
It addition is helps avoid unnecessary aborts that my be caused by accessing
shared memory that would not otherwise cause conflicts, such as relying on a global counter.


\paragraph{Multi-versioning}
The  proposed IR\_VWC\_P  protocol  uses multiple  versions 
(kept in a list) of each  shared object $\ms{X}$.
...
For example when considering permissiveness, by keeping multiple
versions an STM protocol then has a larger time  line to find valid locations
...
In contrast to the work presented in this chapter, none of these papers 
consider virtual wold consistency  as  consistency condition and,
...
Unfortunately a change in one part of the system almost always implies an
impact on another part of the system and because of this a discussion of properties
...
also be used to avoid the revalidation of a transactions read set every time a new read
...
questions such as how many versions to keep and how to clean up old versions.
Given this, it is nearly impossible to choose the ``best'' properties for an STM system
...
Recent research \cite{KR11} has suggested an intereting way to approach this problem by looking at the ammount
of concurrency certain properties (or even combinations of these properties) might allow.
They do this by proving the minimum ammount of expensive synchronization operations that are needed
