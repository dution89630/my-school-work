...
Thread $\ms{T_A}$ runs a protocol that first acquires $\ms{L_1}$ and then $\ms{L_2}$, while thread
...
thread $\ms{T_2}$ nor thread $\ms{T_2}$ will ever acquire both locks.
...
using locks.
In \cite{Ennals06softwaretransactional}, Ennals gives a strong argument for not using non-blocking STM protocols
...
\paragraph{A note about system calls and I/O}
...
that effect things outside of the memory such as input and output to a file
...
Several solutions to this have been proposed on how to allow these operations to be performed
within transactions such as placing all I/O operations
in irrevocable transactions, but this restricts scalability and progress
as only one irrevocable transaction can execute at a time.
Another solution is to perform I/O calls during the commit phase \cite{}, but this also prevents
...
and create a version of it specifically to run efficiently in transactions.
Traditionally STM designers just assume that the programmer is not allowed to perform system or I/O calls
from within his transactions and is what we assume in this thesis.
An important question to answer by future research would be to know what kind of progress can
be ensure for transactions when different types of I/O or system calls can be used.
