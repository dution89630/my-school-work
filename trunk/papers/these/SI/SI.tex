...
\line{C04} \>\> {\bf then} {\bf if} ($\mathit{tmp.time} \geq \mathit{rv}$) \\
...
\line{DA13a} \> $\mathit{abort} \gets \lit{false}$ \\
\line{DA14} \> 
{\bf for each} ($\tuple{\mathit{addr, item}} \in \mathit{ws}$) {\bf do} \\
...
For any T or NT record that is referenced by $\mathit{addr}$, any read operation (transactional or non-transactional) to
$\mathit{addr}$ that dereferences this record will return the same value.
\end{lemma}
\begin{proof}
There are two cases to consider, one when the record dereferenced is of type NT and tow when the record is of type T.
\paragraph{Type NT} In this case using lemma \ref{lemma:si-nochange} and the looking at the code it is easy to see that both the transactional and non-transactional read operations
return the value contained in the $\ms{value}$ field of the NT record dereferenced.
...
\paragraph{Type T} This case is a bit more difficult as the value returned depends on the $\mathit{status}$ of the transaction
...
using lemma \ref{lemma:si-samevalue} at the linearization point, for all the $\ms{addr}$
