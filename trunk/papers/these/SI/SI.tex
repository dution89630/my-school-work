...
In this chapter we will look at how a programmer might use transactions
within his code
in order to suggest a change to the traditional semantics of a transaction.
...
and a transaction to the current discussion of correctness conditions.
Let us start by tring to motivate the difference between transactions
and operations by using an example.
...
semantics of a transaction when considering shared memory.
...
Before giving a more precise defininition on what we mean by this,
th next section will discuss previous solutions on how to deal
...
Transactional memories implementing \emph{weak isolation} make no guarantee
about concurrent transactional and non-transactional memory accesses.
Interestingly most current high performance STM protocols implement weak isolation
as providing stronger guarantees tend to have an impact on performance.
Normally an STM protocol that implemnts weak isolation expects users to avoid
any concurrent interaction between the shared memory outside and inside of
transactions.
In most cases this is because the type of consistency guaranteed is undefined
and in fact usually differes based on the implementation.
Again we avoid this solution as we believe making the user have one section
of memory reserved for his transaction and another section for other shared
operations is not only more of a challenge, but is not consistent with
the concpet of transactions being integrated within a program.

Nevertheless be anticipated and used
...
\pargraph{Specialized transactions}
The previous paragraph mentioned that in some cases a programmer using
an STM that satisfies
weak isolation can deal with concurrency issues between transactional
and non-transactional accesses himself.
In such circumstances the programmer is likely concerning himself
with these issues in the intrest of performance.

In the interest of performance over ease-of-use some solutions
have been proposed allowing the programmer to weaken the atomicity of his transactions.
Usually this involves marking certain reads within a transaction
to not be treated as normal transactional reads.
...
to privitize sections memory that was previously shared to a specific process.
...
to access it outside of transactions.
...
\anote{Need to put in citations for these}
