...
Doing this efficiently is no easy task, \cite{DFGG11} discusses some of the dificulties with STM code complication.
...
different contention managers \cite{RFF07} but there is no definiate answer as 
...
\section{Simplicity}
Give that there are so many different interesting properties to consider
...
In reality though, unfortunately neiter of these goals have yet been realized.
While the basic semantics of a transaction are widely aggreed on
(i.e. transaction atomicity with respect to other transactions), there are many other details to consider,
...
its performance is not good enough to be an attractive alturnative to locks, even considering all the
...
Given the dificulties that exist currently for finding the correct semantics for transactional memory
the goal of this thesis is to take a step twords fixed and easy to understand semantics while suggesting
...
With the grand goal that someday a programmer can write an efficient concurrent program
with nearly as much ease as he would write a sequential one.

Unfortunately we can not immdeately realize this grand goal for there are far too many questions left open when dealing
with the semantics of transactional memory to introduce in this thesis or let alone solve.
...
In this way, each chapter will seperately introduce an area before looking closely at a specific open
...
\subsection{Plan}
Tranditionally research has taken the view that for a protocol to be considered a valid STM
...
(a property important for the speed and scalibility of an STM) and
\emph{permissiveness} (a property preventing a protocl from aborting transactions unnecessarliy)
showing that they are not compatble with the correctness condition
...
that requring the programmer to make these decisions
...
semantics while the thrid chapter suggests expanding the traditional semantics to consider
more issuses that are likely to come up when using STM and provide straightforward
solutions to them.


The second chapter looks at research that takes the view that for transactional memory to
...
Specifically it looks at the problem of aborted transactions and suggests that
the programmer should not have to be aware of them at all, following this suggestion it introduces a protocol
...
Taking just the traditional definition of transactions from the first chapter only takes a very limited view of how
...
to be familir whith how different STM protocols specfically interact with 
...
A protocol protocol ensuring this property is designed.
...
The focus of the fourth chapter is to just to look at how to merge the research
from the previous chapter.
Afterall it is nice to give different suggestions for how the semantics
of transactions should be modified, but it is less meaningfull if they are not compatable with
...
The fifth chapter looks into the idea of providing efficient abstractions
as libraires for programmers to reuse from within their transactions.
...
to write high performance implementations designed specfically for transactions.
It specifcally looks into deisigned efficent data strcutres implemnting the map abstraction.


By solving small problems in each one of these areas hopefully this thesis will
help move forward the ease of use of transactional memory and mentioning
