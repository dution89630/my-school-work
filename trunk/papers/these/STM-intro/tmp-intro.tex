...
Using locks in such a way is often referred to as ``course-grained'' locking.
...
as $compare\_and\_swap$ and do not suffer some of the scalability or progress
...
but make no considerations about the difficulties of writing concurrent programmer.
...
have been performed that (under the parameters of their studies) find users can create concurrent programs
...
Without a proper consistency criterion the programmer is lost, it ensures that transactions do execute
...
For example a transaction that took place long after a previous transaction committed can
...
In an implementation that uses \emph{write buffering} when a transaction wants to perform
...
Like everything, there are some advantages and disadvantages to both solutions, and depend on how the rest of the 
...
workloads because contention is being introduced when there are no conflicts and can limit scalibility.
...
In reality though, unfortunately neither of these goals have yet been realized.
...
Given the difficulties that exist currently for finding the correct semantics for transactional memory
the goal of this thesis is to take a step towards fixed and easy to understand semantics while suggesting
...
Unfortunately we can not immediately realize this grand goal for there are far too many questions left open when dealing
...
believed are the main areas of transactional memory research that take the
ease-of-use as a primary concern.
In this way, each chapter will separately introduce an area before looking closely at a specific open
...
with each other are left up to the programmer.
...
more issues that are likely to come up when using STM and provide straightforward
solutions to them.
...
It attempts to answer the question, ``how should the system act when shared memory
...
It specifically looks into designed efficient data structures implementing the map abstraction.
