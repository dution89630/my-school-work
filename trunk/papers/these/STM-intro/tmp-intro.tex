...
locking is by far the most widely used abstraction to
...
restarted after an abort [aborts are described in the following section]) and can therefore be performed more efficiently than shared accesses.
...
A further complication with shared memory accesses is defining what happens when shared memory is accessed
...
\subsection{Correctness}\label{sec:int-correct}
...
and even different than other concurrent objects,
...
Often one type performs better than the other depending on the workload.
...
% Alternatively Ennals \cite{Ennals05efficientsoftware} suggests that lazy detection is preferable as
...
\subsubsection{Dealing with conflicts}\label{sec:intro-contention}
...
\end{defin}
As this definition will be used in this thesis as a goal for how STM should be viewed,
instead of staring with a complex definition that directly takes into account the complex details of the concurrent system
(which would complicate things from the beginning),
we start with this vague definition that will be used as a guide to follow throughout the thesis where,
throughout the chapters, these complex details are examined.
Following this, we will define (in the same light) how a transaction will be viewed.
...
\begin{defin}\label{def:trans-def}
...
and possible solution from the area, each helping to move towards a solution of our goal defined in Definition \ref{def:stm-def}.
...
Following this model, chapter \ref{chap:VWC} looks at the specific properties of \emph{invisible reads}
...
Chapters \ref{chap:UC} and \ref{chap:SI}  look at research that takes the approach
...
Chapter \ref{chap:UC} suggests looking at ways to simplify the traditional STM
...
Specifically, chapters \ref{chap:UC} looks at research that takes the view that for transactional memory to
...
The area of research looked at in chapter \ref{chap:SI} suggests expanding the basic semantics of transactions.
Taking just the traditional definition of transactions from the chapter \ref{chap:VWC} only leaves a very limited view of how
...
Chapter \ref{chap:STMlib} looks into the idea of providing efficient abstractions
